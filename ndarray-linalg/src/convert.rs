...
where
    ...
    D: Dimension,
{
    unsafe { ArrayBase::<So, D>::build_uninit(a.dim(), |mut b| a.assign_to(&mut b)).assume_init() }
}
...
{
    let shape = match l {
        MatrixLayout::C { row, lda } => (row as usize, lda as usize).into_shape(),
        MatrixLayout::F { col, lda } => (lda as usize, col as usize).f(),
    };
    unsafe { ArrayBase::<So, _>::build_uninit(shape, |mut b| a.assign_to(&mut b)).assume_init() }
}
